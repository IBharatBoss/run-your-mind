 ...
 * 1. Validates premium status locally (no network call), or bypasses for free demo items.
...
async function handleBookClick(articleKey) {
    ...
    const isFree = cachedItem && cachedItem.is_free === true;
    ...
    if (cachedItem && cachedItem.file_url) {
        const fileUrl = cachedItem.file_url;
        const finalUrl = fileUrl.includes('?') ? fileUrl + '&id=' + articleKey : fileUrl + '?id=' + articleKey;
        window.open(finalUrl, 'ReaderTab');
        return;
    }
